  ...
  Ident.Sattr.fold
    (fun attr acc ->
      ...
      let rec aux l =
        ...
        | p :: r -> (
          try
            let s = Strings.remove_prefix p attr in
            s :: acc
          with
          | Not_found -> aux r)
      in
...
let search_attrs callback =
  let rec aux acc f =
    if f.t_ty <> None then
      acc
    else if Ident.Sattr.mem Term.stop_split f.Term.t_attrs then
      acc
    else
...
let goal_expl_task ~root task =
  ...
  let info =
    ...
    concat_expls
      (if res <> [] && not root then
        res
      else
        collect_expls gid.Ident.id_attrs)
  in
  let info =
    match info with
    | Some i -> i
...
(* let dict_table = Hashtbl.create 17 let dict_count = ref 0 let
   reverse_ident_table = Hashtbl.create 17 let reverse_dict_count = ref 0

   let reset_dict () = Hashtbl.clear dict_table; Hashtbl.clear
   reverse_ident_table; dict_count := 0; reverse_dict_count := 0 *)

(* {3 direct table to read shapes from strings} *)

(* let get_name s b i = try while !i < String.length s do match String.get s !i
   with | ')' -> incr i; raise Exit | c -> incr i; Buffer.add_char b c done;
   invalid_arg "Termcode.get_name: missing closing parenthesis" with Exit -> let
   id = Buffer.contents b in Hashtbl.add dict_table !dict_count id; (*
   Format.eprintf "%d -> %s@." !dict_count id; *) incr dict_count; id

   let get_num s n i = try while !i < String.length s do match String.get s !i
   with | ')' -> incr i; raise Exit | '0'..'9' as c -> incr i; n := !n * 10 +
   (Char.code c - Char.code '0') | _ -> invalid_arg "Termcode.get_num: decimal
   digit expected" done; invalid_arg "Termcode.get_num: missing closing
   parenthesis" with Exit -> try Hashtbl.find dict_table !n with Not_found ->
   invalid_arg ("Termcode.get_num: invalid ident number " ^ string_of_int !n) *)

(* let get_id s i = if !i >= String.length s then invalid_arg "Termcode.get_id:
   missing closing parenthesis"; match String.get s !i with | '0'..'9' as c ->
   let n = ref (Char.code c - Char.code '0') in incr i; get_num s n i | ')' ->
   invalid_arg "Termcode.get_id: unexpected closing parenthesis" | c -> let b =
   Buffer.create 17 in Buffer.add_char b c; incr i; get_name s b i *)

(* let store_id s i = let b = Buffer.create 17 in try while !i < String.length s
   do match String.get s !i with | ')' -> incr i; raise Exit | c -> incr i;
   Buffer.add_char b c done; invalid_arg "Termcode.store_id: missing closing
   parenthesis" with Exit -> let id = Buffer.contents b in try let n =
   Hashtbl.find reverse_ident_table id in string_of_int n with Not_found ->
   Hashtbl.add reverse_ident_table id !reverse_dict_count; incr
   reverse_dict_count; id *)

module Common = struct
  let sign fmt n = if n then Format.pp_print_char fmt '-'
  ...
  let const_v1 fmt c =
    ...
    | ConstReal
        { rl_kind = k; rl_real = { rv_sig = i; rv_pow2 = p2; rv_pow5 = p5 } }
      -> (
      ...
      | RLitUnk
      | RLitDec _ ->
        Format.fprintf fmt "%s.0e%sp%s" (BigInt.to_string i)
          (BigInt.to_string p5) (BigInt.to_string p2)
      | RLitHex _ ->
        Format.fprintf fmt "%a.0p%se%s"
          (Number.print_in_base 16 None)
          i (BigInt.to_string p2) (BigInt.to_string p5))
    | ConstStr c -> print_string_def fmt c
  ...
  let const_v2 fmt c =
    ...
    | ConstInt { il_kind = _; il_int = i } ->
      Format.pp_print_string fmt (BigInt.to_string i)
    | ConstReal
        { rl_kind = _; rl_real = { rv_sig = i; rv_pow2 = p2; rv_pow5 = p5 } } ->
      Format.fprintf fmt "%s.p%se%s" (BigInt.to_string i) (BigInt.to_string p2)
        (BigInt.to_string p5)
    | ConstStr c -> print_string_def fmt c
end
...
let shape_of_string =
  ...
  fun ~version s ->
    if is_bound_sum_shape_version version then (
      ...
      (* most consecutive shapes have very long common suffixes, so we look for
         some sharing with the previously computed shape in order to reduce the
         memory footprint of large sessions *)
      let c, lc =
        let lc = List.length !cache in
        let ls = List.length s in
        (* this avoids reverting a uselessly long list below *)
        if lc > ls then
          (Lists.chop (lc - ls) !cache, ls)
        else
          (!cache, lc)
      in
      let rec aux c s n =
...
let debug =
  Debug.register_info_flag "session_pairing"
    ~desc:
      "Print@ debugging@ messages@ about@ reconstruction@ of@ session@ trees@ \
       after@ modification@ of@ source@ files."
...
module Shape = struct
  (* similarity code of terms, or of "shapes"
     ...
     Forall(Int,App(infix_gteq,App(infix_st,Tvar 0,Tvar 0),Const(0))) i.e: de
     bruijn indexes, first-order term *)
  ...
  let check () = if Buffer.length shape_buffer >= 1024 then raise ShapeTooLong
  ...
  let decoded pr id =
    Ident.(
      match sn_decode id.id_string with
  ...
  let ident_v4 h id =
    let x =
      try Ident.Mid.find id !h with
      | Not_found ->
        let s = id.Ident.id_string in
        h := Ident.Mid.add id s !h;
        s
    in
    push x
  ...
  let rec pat_rename_alpha c h p =
    match p.pat_node with
    | Pvar v -> vs_rename_alpha c h v
    | Pas (p, v) ->
      vs_rename_alpha c h v;
      pat_rename_alpha c h p
  ...
  let rec t_shape ~version pr c m t =
    ...
    | Tconst c ->
      pushc tag_const;
      const_shape version c
    | Tvar v ->
      pushc tag_var;
      ident_shape version pr m v.vs_name
    | Tapp (s, l) ->
      pushc tag_app;
    ...
    | Tquant (q, b) ->
      ...
      (* argument first, intentionally, to give more weight on A in forall
         x,A *)
      fn f1;
      let hq =
        match q with
        ...
        | SV6 ->
          ...
          let do_td td =
            ...
            | Theory.Decl d -> (
              match d.d_node with
              | Dparam _ls -> ()
              | Dprop (_, _pr, f) -> t_shape ~version pr c m f
              | _ -> ())
            | _ -> ()
          in
...
module Gshape = struct
  ...
  (* [hyp_shape] associate a shape to its index in the file; [index_sh]
     associate an index to a shape; [n] is the last shape variable *)
  type gshape = {
  ...
  let create () = { hyp_shape = Mstr.empty; index_sh = Mint.empty; n = 0 }
  ...
  let add_and_prepend gs current_shape s : unit =
    (* Do not save empty: could be confused with end of assingments (TODO to be
       improved) *)
    if s = "" then
      ()
    else
  ...
  let write_shape_to_file s csh =
    Mint.iter
      (fun _ s ->
        Compress.Compress_z.output_string csh s;
  ...
  let t_bound_shape_task gs ~version ~expl t =
    ...
    let do_d d =
      ...
      | Dprop (_, _pr, f) ->
        let sh =
          (try Shape.t_shape ~version pr c m f with
          | Shape.ShapeTooLong -> ());
          Buffer.contents Shape.shape_buffer
        in
        Buffer.clear Shape.shape_buffer;
    ...
    (* TODO don't save empty shapes as \n as it could be confused with end of
       assignments *)
    let expl =
      if expl = "" then
        "empty_shape"
...
module Checksum = struct
  let char (_, _, _, buf, pos, _) c =
    incr pos;
    Buffer.add_char buf c

  let int ((_, _, _, buf, pos, _) as b) i =
    char b 'i';
    let s = string_of_int i in
    pos := !pos + String.length s;
    Buffer.add_string buf s

  let string ((_, _, _, buf, pos, _) as b) s =
    char b '"';
  ...
  let ident_v1, clear_ident_v1 =
    ...
    ( (fun b id ->
        int b
          (try Ident.Hid.find hident id with
          | Not_found ->
            incr c;
            Ident.Hid.add hident id !c;
            !c)),
      fun () ->
        Ident.Hid.clear hident;
        c := 0 )

  let ident_v2 ((_, c, m, _, pos, m') as b) id =
    let i =
      match Ident.Mid.find_opt id !m with
      | Some i -> i
      | None ->
        incr c;
        m := Ident.Mid.add id !c !m;
        !c
    in
    let old_pos = !pos in
    int b i;
    let len = !pos - old_pos in
    match Ident.Mid.find_opt id !m' with
    | Some (_, l) -> m' := Ident.Mid.add id (i, (old_pos, len) :: l) !m'
    | None -> m' := Ident.Mid.add id (i, [ (old_pos, len) ]) !m'

  let ident ((v, _, _, _, _, _) as b) id =
    match v with
    | SV1
    | SV2
    | SV3 ->
  ...
  let const (v, _, _, buf, count, _) c =
    ...
    | SV5 ->
      Common.const_v1 fmt c
    | SV6 -> Common.const_v2 fmt c);
    let old_len = Buffer.length buf in
    Format.pp_print_flush fmt ();
    count := !count + Buffer.length buf - old_len
  ...
  let vsymbol ((v, _, _, _, _, _) as b) vs =
    match v with
    | SV1
    | SV2
    | SV3 ->
  ...
  let decl b d =
    ...
    | Decl.Dtype ts ->
      ...
      tysymbol b ts
    | Decl.Ddata ddl ->
      let constructor b (ls, l) =
        lsymbol b ls;
        list (option lsymbol) b l
      in
      let data_decl b (ts, cl) =
        tysymbol b ts;
        list constructor b cl
      in
      char b 'D';
      list data_decl b ddl
    | Decl.Dparam ls ->
      char b 'R';
      lsymbol b ls
    | Decl.Dlogic ldl ->
      ...
      in
      char b 'L';
      list logic_decl b ldl
    | Decl.Dind (s, idl) ->
      let clause b (pr, f) =
        ident b pr.Decl.pr_name;
        term b f
      in
      let ind_decl b (ls, cl) =
        lsymbol b ls;
        list clause b cl
      in
      char b 'I';
  ...
  let tdecl
      ?(toto : (checksum * (int * (int * int) list) Ident.Mid.t) Hdecl.t option)
      (v, c, m, buf) d : unit =
    let b = (v, c, m, buf, ref 0, ref Ident.Mid.empty) in
    ...
    | Theory.Decl d -> (
      ...
      | Some map ->
        decl b d
        (* ( match Hdecl.find_opt map d with | Some (s, map) -> let s =
           Ident.Mid.fold_left (fun s id (i, occurences) -> let i' = try
           Ident.Mid.find id !m with | Not_found -> incr c; m := Ident.Mid.add
           id !c !m; !c in if i = i' then s else s (* let sub = "i" ^
           string_of_int i' in *) (* let rec subst offset s l = *) (* match l
           with *) (* | [] -> s *) (* | (pos, len) :: tl -> *) (* let _ = *) (*
           Format.fprintf Format.std_formatter "pos : %d\n" pos *) (* in *) (*
           let _ = *) (* Format.fprintf Format.std_formatter "len : %d\n" pos *)
           (* in *) (* let _ = *) (* Format.fprintf Format.std_formatter "offset
           : %d\n" *) (* offset *) (* in *) (* let _ = *) (* Format.fprintf
           Format.std_formatter "s : %s\n" s *) (* in *) (* let pos = pos -
           offset in *) (* let offset = offset + (len + pos) in *) (* let s' =
           *) (* if pos + len >= String.length s then *) (* "" *) (* else *) (*
           "" *) (* (* subst offset *) *) (* (* (String.sub s (pos + len) *) *)
           (* (* (String.length s - pos - len)) *) *) (* (* tl *) *) (* in *) (*
           s *) (* (* "" *) *) (* (* String.sub s 0 pos ^ sub ^ s' *) *) (* in
           *) (* subst 0 s (List.rev occurences)) *)) s map in Buffer.add_string
           buf s | None -> let len_old = Buffer.length buf in let m' = ref
           Ident.Mid.empty in decl (v, c, m, buf, ref 0, m') d; let len =
           Buffer.length buf in let s = Buffer.sub buf len_old (len - len_old)
           in Hdecl.add map d (s, !m')))*))
    | Theory.Use th
    | Theory.Clone (th, _) ->
      char b 'C';
      ident b th.Theory.th_name;
      list string b th.Theory.th_path
    | Theory.Meta (m, mal) ->
      char b 'M';
      meta b m;
      list meta_arg b mal
  ...
  (* not used anymore
     ...
     [tdecl] function for it. [Use] in a theory requires a recursive call (as
     below), [Use] in a task is just a witness declaration.

     let rec tdecl b d = match d.Theory.td_node with | Theory.Decl d -> decl b d
     | Theory.Use th -> char b 'U'; ident b th.Theory.th_name; list string b
     th.Theory.th_path; string b (theory_v2 th) | Theory.Clone (th, _) -> char b
     'C'; ident b th.Theory.th_name; list string b th.Theory.th_path |
     Theory.Meta (m, mal) -> char b 'M'; meta b m; list meta_arg b mal

     and theory_v2_aux t = let c = ref 0 in let m = ref Ident.Mid.empty in let b
     = Buffer.create 8192 in List.iter (tdecl (CV2,c,m,b)) t.Theory.th_decls;
     let dnew = Digest.string (Buffer.contents b) in Digest.to_hex dnew

     and theory_v2 = let table = Ident.Wid.create 17 in fun t -> try
     Ident.Wid.find table t.Theory.th_name with Not_found -> let v =
     theory_v2_aux t in Ident.Wid.set table t.Theory.th_name v; v

     let theory ~version t = match version with | CV1 -> assert false | CV2 ->
     theory_v2 t *)
    ...
    let task_hd t (cold, mold, dold) =
      ...
      let mnew =
        ...
        | Theory.Decl { Decl.d_news = s } ->
          Ident.Sid.fold
            (fun id a -> Ident.Mid.add id (Ident.Mid.find id !m) a)
            s mold
        | _ -> !m
      in
      (!c, mnew, dnew)
  ...
  (* WARNING: The occurence of [Trans.fold] in [task_v3] needs to be executed
     ...
     executed once and shared for all functions. So, in particular, don't add
     arguments to task_v3 here. *)
  let task_v3 =
    let c = ref 0 in
    let m = ref Ident.Mid.empty in
    let b = Buffer.create 8192 in
    let task_hd toto t (cold, mold, dold) =
      c := cold;
      let mo = DMid.get mold in
      m := mo;
      tdecl ?toto (SV6, c, m, b) t.Task.task_decl;
      Buffer.add_string b (Digest.to_hex dold);
      let dnew = Digest.string (Buffer.contents b) in
      Buffer.clear b;
      let mnew =
        match t.Task.task_decl.Theory.td_node with
        | Theory.Decl { Decl.d_news = s } ->
          (* as metas might have been put on some symbols before their
             declaration, ignore such symbols *)
          let s = Ident.Mid.set_diff s mo in
          Ident.Mid.mapi
            (fun id () ->
              try Ident.Mid.find id !m with
              | Not_found -> failwith "AH BON ?")
            s
        | _ -> Ident.Mid.set_diff !m mo
      in
      (!c, DMid.union mold mnew, dnew)
    in
    let tr toto =
      Trans.fold (task_hd toto) (0, DMid.empty (), Digest.string "")
    in
    fun toto t ->
      let _, _, dnew = Trans.apply (tr toto) t in
      Digest.to_hex dnew

  let task ~version ?toto t =
    match version with
    | SV1
    | SV2
    | SV3 ->
...
let task_checksum ?toto ?(version = current_sum_shape_version) t =
  (* let version = match version with | SV1 | SV2 | SV3 -> CV1 | 4 | 5 -> CV2 |
     6 -> CV3 | _ -> assert false (* FIXME !!! *) in *)
  (* let tim = Unix.gettimeofday () in *)
  let s = Checksum.task ?toto ~version t in
  (* let tim = Unix.gettimeofday () -. tim in time := !time +. tim;
...
(* we have an ordered list of new subgoals
   ...
   where each new goal is mapped either to - None: no pairing at all - Some (h,
   false): exact matching (equal checksums) - Some (h, true): inexact matching
   (goal obsolete) *)
...
module Pairing (Old : S) (New : S) = struct
  let rec lcp n s1 s2 =
    if String.length s1 <= n || String.length s2 <= n then
      n
  ...
  let rec count_same_sorted n l1 l2 =
    match (l1, l2) with
    | hd1 :: tl1, hd2 :: _ when hd1 < hd2 -> count_same_sorted n tl1 l2
    | hd1 :: _, hd2 :: tl2 when hd1 > hd2 -> count_same_sorted n l1 tl2
    | hd1 :: tl1, hd2 :: tl2 when hd1 = hd2 -> count_same_sorted (n + 1) tl1 tl2
    | _ -> n
  ...
  let lcp_list (sh1, sl1) (sh2, sl2) = (lcp sh1 sh2, count_common sl1 sl2)
  ...
  let mk_node table g =
    let s =
      ...
      | New g ->
        ...
        (sh, List.sort Int.compare l)
    in
    let rec n = { prev = n; shape = s; elt = g; next = n; valid = true } in
    n
  ...
  let associate oldgoals newgoals =
    let table =
      { table_old = Array.of_list oldgoals; table_new = Array.of_list newgoals }
    in
    (* set up an array [result] containing the solution [new_goal_index g]
       returns the index of goal [g] in that array *)
    let new_goal_index = Hid.create 17 in
    let result =
      let make i newg =
        Hid.add new_goal_index (New.name newg) i;
        (newg, None)
      in
      Array.mapi make table.table_new
    in
    let new_goal_index newg =
      try Hid.find new_goal_index (New.name newg) with
    ...
    let newgoals =
      ...
      for newi = Array.length table.table_new - 1 downto 0 do
        ...
        with
        | Not_found -> acc := mk_node table (New newi) :: !acc
      done;
      !acc
    in
    ...
    if allgoals <> [] then (
      let module E = struct
        let dummy =
          let n = List.hd allgoals (* safe *) in
          ((0, 0), (n, n))
