...
let debug =
  Debug.register_info_flag "session_itp"
    ~desc:
      "Pring@ debugging@ messages@ about@ Why3@ session@ creation,@ reading@ \
       and@ writing."
...
let print_proofNodeID fmt id = Format.fprintf fmt "proofNodeID<%d>" id
let print_proofAttemptID fmt id = Format.fprintf fmt "proofAttemptID<%d>" id
...
let fprintf_any fmt a =
  match a with
  | AFile f ->
    Format.fprintf fmt "<AFile [%a]>" Sysutil.print_file_path f.file_path
...
type session = {
  ...
  pn_state : bool Hpn.t;
  decl_checksums :
    (Termcode.checksum * (int * (int * int) list) Ident.Mid.t) Decl.Hdecl.t;
}
...
module Sshape = struct
  ...
  let add_shape s goal_id shape =
    ...
    match shape with
    | Old_shape shape -> Hpn.add sshape.session_shape_table goal_id shape
    | Bound_shape shape ->
  ...
  let get_shape s goal_id =
    if is_bound_sum_shape_version s.shapes.shape_version then
      let shape =
        try Hpn.find s.shapes.session_bound_shape_table goal_id with
        | Not_found -> empty_bound_shape
      in
      Bound_shape shape
    else
      let shape =
        try Hpn.find s.shapes.session_shape_table goal_id with
        | Not_found -> empty_shape
      in
      Old_shape shape
  ...
  let add_sum s goal_id sum = Hpn.add s.shapes.session_sum_table goal_id sum
...
let session_iter_proof_attempt f s = Hint.iter f s.proofAttempt_table

(* This is not needed. Keeping it as information on the structure type tree = {
   tree_node_id : proofNodeID; tree_goal_name : string; tree_proof_attempts :
   proof_attempt list; (* proof attempts on this node *) tree_transformations :
   (transID * string * tree list) list; (* transformations on this node *) } *)

(* let rec get_tree s id : tree = let t = Hint.find s.proofNode_table id in let
   pal = Hprover.fold (fun _ pa acc -> pa.proofa_attempt::acc) t.proofn_attempts
   [] in let trl = List.map (get_trans s) t.proofn_transformations in {
   tree_node_id = id; tree_goal_name = t.proofn_name.Ident.id_string;
   tree_proof_attempts = pal; tree_transformations = trl; }

   and get_trans s id = let tr = Hint.find s.trans_table id in (id,
   tr.transf_name, List.map (get_tree s) tr.transf_subtasks) *)

(* let get_theories s = Hstr.fold (fun _fn f acc -> let c = List.map (fun th ->
   (th.theory_name.Ident.id_string, th.theory_goals)) f.file_theories in (f,c)
   :: acc) s.session_files [] *)
...
(* let get_node (s : session) (n : int) = let _ = Hint.find s.proofNode_table n
   in n

   let get_trans (s : session) (n : int) = let _ = Hint.find s.trans_table n in
   n *)
...
let get_proof_attempt_node (s : session) (id : proofAttemptID) =
  try Hint.find s.proofAttempt_table id with
  | Not_found -> raise BadID

let get_proofNode (s : session) (id : proofNodeID) =
  try Hint.find s.proofNode_table id with
  | Not_found -> raise BadID

let get_task s id = Hpn.find s.session_raw_tasks id

let get_task_name_table s n =
  let t = get_task s n in
  let table =
    try Hpn.find s.session_task_tables n with
    | Not_found ->
...
let get_transfNode (s : session) (id : transID) =
  try Hint.find s.trans_table id with
  | Not_found -> raise BadID
...
let goal_is_detached s pn =
  try
    let (_ : Task.task) = get_task s pn in
    false
  with
  | Not_found -> true

let transf_is_detached s tn = (get_transfNode s tn).transf_is_detached
...
(* let set_obsolete s paid b = let pa = get_proof_attempt_node s paid in
   pa.proof_obsolete <- b *)
...
let get_transformation s pid t args =
  ...
  List.find
    (fun tr_id ->
      get_transf_name s tr_id = t
      && get_transf_args s tr_id = args
      && not (is_detached s (ATn tr_id)))
    sub_transfs
...
let rec fold_all_any_of_transn s f acc trid =
  ...
  let acc =
    List.fold_left (fold_all_any_of_proofn s f) acc tr.transf_subtasks
  in
  f acc (ATn trid)
...
and fold_all_any_of_proofn s f acc pnid =
  ...
  let acc =
    List.fold_left
      (fun acc trid -> fold_all_any_of_transn s f acc trid)
      acc pn.proofn_transformations
  in
  let acc =
    Hprover.fold (fun _p paid acc -> f acc (APa paid)) pn.proofn_attempts acc
  in
  f acc (APn pnid)
...
let fold_all_any_of_file s f acc file =
  let acc =
    List.fold_left (fold_all_any_of_theory s f) acc file.file_theories
  in
  f acc (AFile file)
...
let rec fold_all_sub_goals_of_proofn s f acc pnid =
  ...
  let acc =
    List.fold_left
      (fun acc trid ->
        let tr = get_transfNode s trid in
        List.fold_left (fold_all_sub_goals_of_proofn s f) acc tr.transf_subtasks)
      acc pn.proofn_transformations
  in
  f acc pn

let goal_iter_proof_attempt s f g =
  fold_all_sub_goals_of_proofn s
    (fun _ pn ->
      Hprover.iter
        (fun _ pa ->
...
(* let theory_iter_proofn s f th = fold_all_sub_goals_of_theory s (fun _ -> f)
   () th *)
...
let file_iter_proof_attempt s f file =
  List.iter (theory_iter_proof_attempt s f) (file_theories file)
...
let get_any_parent s a =
  ...
  | APa pa -> Some (APn (get_proof_attempt_node s pa).parent)
...
let print_proof_attempt fmt pa =
  fprintf fmt "@[<h>%a tl=%g %a@]" Whyconf.print_prover pa.prover
  ...
  fprintf fmt
    ...
    (Hprover.fold
       (fun _key e l ->
         ...
         e :: l)
       pn.proofn_attempts [])
    (Pp.print_list Pp.semi (print_trans_node s))
    pn.proofn_transformations
...
and print_trans_node s fmt id =
  ...
  fprintf fmt "@[<hv 1> Trans %s;@ args %a;@ parent %s;@ [%a]@]" name
    (Pp.print_list Pp.colon pp_print_string)
    args parent
    (Pp.print_list Pp.semi (print_proof_node s))
    l

let print_theory s fmt th : unit =
  fprintf fmt "@[<hv 2> Theory %s;@ [%a]@]" th.theory_name.Ident.id_string
    (Pp.print_list Pp.semi (fun fmt a -> print_proof_node s fmt a))
    th.theory_goals

let print_file s fmt (file, thl) =
  fprintf fmt "@[<hv 2> File [%a];@ [%a]@]" Sysutil.print_file_path
    file.file_path
    (Pp.print_list Pp.semi (print_theory s))
    thl

let print_s s fmt = fprintf fmt "@[%a@]" (Pp.print_list Pp.semi (print_file s))

let _print_session fmt s =
  let l =
    Hfile.fold (fun _ f acc -> (f, f.file_theories) :: acc) (get_files s) []
  in
...
let empty_session ?sum_shape_version ?from dir =
  ...
  in
  let version, has_shapes =
    match (sum_shape_version, from) with
    | None, None -> (Termcode.current_sum_shape_version, false)
    | Some v, None -> (v, true)
    | None, Some s -> (s.shapes.shape_version, s.shapes.has_shapes)
    | Some _, Some _ ->
      failwith
        "Session_itp.empty_session: cannot use both optional arguments at the \
         same time"
  in
  let empty_shapes =
    {
  ...
  | Some from -> Sshape.copy_global_shape from.shapes empty_shapes);
  {
    ...
    decl_checksums = Hdecl.create 109;
  }
...
let add_proof_attempt session prover limit state ~obsolete
    (edit : Sysutil.file_path option) parentID =
...
let graft_proof_attempt ?file (s : session) (id : proofNodeID)
    (pr : Whyconf.prover) ~limit =
  let pn = get_proofNode s id in
  try
    let id = Hprover.find pn.proofn_attempts pr in
    let pa = Hint.find s.proofAttempt_table id in
    let pa = { pa with limit; proof_state = None; proof_obsolete = false } in
    (* Hprover.replace pn.proofn_attempts pr id; useless *)
    Hint.replace s.proofAttempt_table id pa;
    id
  with
  | Not_found -> add_proof_attempt s pr limit None ~obsolete:false file id

(* [mk_proof_node s n t p id] register in the session [s] a proof node of
   proofNodeID [id] of parent [p] of task [t] *)
let mk_proof_node ~expl (s : session) (n : Ident.ident) (t : Task.task)
  ...
  let toto = Some s.decl_checksums in
  let sum = Termcode.task_checksum ?toto ~version:s.shapes.shape_version t in
  Sshape.add_sum s node_id sum;
  let a = Sshape.compute_and_add_shape s ~expl node_id t in
  a
...
let th_proved s th =
  try Hid.find s.th_state th.theory_name with
  | Not_found ->
...
let file_proved s f =
  try Hfile.find s.file_state f.file_id with
  | Not_found ->
...
(* [update_goal_node c id] update the proof status of node id update is
   propagated to parents when required. *)
...
let update_theory_node notification s th =
  ...
  if proved <> th_proved s th then (
    ...
    notification (ATh th);
    try
      let p = theory_parent s th in
      update_file_node notification s p
    with
    | Not_found when not (Debug.test_flag Debug.stack_trace) ->
...
let rec update_goal_node notification s id =
  ...
  let proved =
    List.exists
      (fun tr -> (not (transf_is_detached s tr)) && tn_proved s tr)
      tr_list
    || List.exists
         (fun pa -> (not (goal_is_detached s pa.parent)) && pa_ok pa)
         pa_list
  in
  if proved <> pn_proved s id then (
    (* too noisy, uncomment if you really need it Debug.dprintf debug "[Session]
       setting goal node %a to status proved=%b@." print_proofNodeID id
       proved; *)
...
and update_trans_node notification s trid =
  ...
  let proved =
    List.for_all (fun pn -> goal_is_detached s pn || pn_proved s pn) proof_list
  in
...
let remove_subtree ~(notification : notifier) ~(removed : notifier) s (n : any)
  ...
  let remove (n : any) =
    ...
    | APn pn -> (
      ...
      | Theory th -> th.theory_goals <- List.filter (( <> ) pn) th.theory_goals
      | Trans tr ->
...
let float_attribute_def field r def =
  try float_of_string (List.assoc field r.Xml.attributes) with
  | Not_found
  | Invalid_argument _ ->
    def

let int_attribute_def field r def =
  try int_of_string (List.assoc field r.Xml.attributes) with
  | Not_found
  | Invalid_argument _ ->
    def

let string_attribute_def field r def =
  try List.assoc field r.Xml.attributes with
  | Not_found -> def

let string_attribute_opt field r =
  try Some (List.assoc field r.Xml.attributes) with
  | Not_found -> None
...
let load_result a (path, acc) r =
  ...
  | "result" ->
    ...
    in
    let time =
      try float_of_string (List.assoc "time" r.Xml.attributes) with
      | Not_found -> 0.0
    in
    let steps =
      try int_of_string (List.assoc "steps" r.Xml.attributes) with
      | Not_found -> -1
    in
    let res =
      {
...
let load_option attr g =
  try Some (List.assoc attr g.Xml.attributes) with
  | Not_found -> None
  ...
  let attrs =
    List.fold_left
      (fun acc label ->
        ...
        | _ -> acc)
      Ident.Sattr.empty elt.Xml.elements
  in
  let preid = Ident.id_fresh ~attrs name in
  Ident.id_register preid

(* [load_goal s op p g id] loads the goal of parent [p] from the xml [g] of
   nodeID [id] into the session [s] *)
let rec load_goal session old_provers parent g id =
  match g.Xml.name with
  | "goal" ->
    let gname = load_ident g in
    (* even if sum and shape are not in the XML file but in the shape file,
       these attributes are there thanks to ~fixattr on Xml.from_file *)
    ...
    let shape =
      Termcode.shape_of_string ~version:session.shapes.shape_version
        (try List.assoc "shape" g.Xml.attributes with
        | Not_found -> "")
    in
  ...
  | "label" -> ()
  | s -> Loc.warning "Session.load_goal: unexpected element '%s'@." s

(* [load_proof_or_transf s op pid a] load either a proof attempt or a
   transformation of parent id [pid] from the xml [a] into the session [s] *)
and load_proof_or_transf session old_provers pid a =
  ...
  | s ->
    Loc.warning
      "[Warning] Session.load_proof_or_transf: unexpected element '%s'@." s
...
let load_file session old_provers f =
  ...
  | "file" ->
    ...
    let path, ft =
      List.fold_left
        (load_theory session fid old_provers)
        (Sysutil.empty_path, []) f.Xml.elements
    in
    let path =
      if Sysutil.is_empty_path path then
        match fn with
        | Some fn ->
          ...
          Debug.dprintf debug "Loaded path from concrete file name: %a@."
            Sysutil.print_file_path l;
          l
        | None -> assert false
      else
        path
    ...
    let fmt =
      ...
      let def_fmt =
        Env.get_format (Format.asprintf "%a" Sysutil.print_file_path path)
      in
      string_attribute_def "format" f def_fmt
    in
    let mf =
      {
        file_id = fid;
  ...
  | "prover" -> (
    ...
    with
    | Failure _ ->
      Loc.warning "Session.load_file: unexpected non-numeric prover id '%s'@."
        id;
      old_provers)
  | s ->
...
(* let get_version (xml: Xml.t) = match xml.Xml.content.Xml.name with |
   "why3session" -> let shape_version = int_attribute_def "shape_version"
   xml.Xml.content 1 in shape_version | s -> Loc.warning "Session.load_session:
   unexpected element '%s'@." s; Termcode.current_shape_version *)
...
module ReadShapes (C : Compress.S) = struct
  ...
  let read_sum_and_shape ch =
    ...
    (try
       ...
       if nsum <> 32 then C.really_input ch sum nsum (32 - nsum)
     with
    | End_of_file ->
      raise
        (ShapesFileError
      ...
      try
        ...
        try
          ...
          if Termcode.is_bound_sum_shape_version sv then (
            try read_global_buffer gs ch with
            | ShapesFileError msg ->
              Loc.warning
                "Error while reading shape file: %s. Continuing without shapes"
                msg;
      ...
      try
        ...
        let attrs =
          try
            ...
            attrs
          with
          | Not_found -> ("sum", sum) :: attrs
        in
        ("shape", shape) :: attrs
      with
      | ShapesFileError msg ->
        Loc.warning
          "Error while reading shape file: %s. Continuing without shapes." msg;
        sum_shape_version := None;
        attrs)
    | _ -> attrs
...
let read_file_session_and_shapes gs dir xml_filename =
  ...
  if Sys.file_exists compressed_shape_filename then
    if Compress.compression_supported then
      ...
      let () =
        Loc.warning
          "Could not read goal shapes because Why3 was not compiled with \
           compress support@."
      in
...
let build_session ?sum_shape_version (s : session) xml : unit =
  ...
  | "why3session" ->
    let sv =
      try
        Some
          (Termcode.string_to_sum_shape_version
             (List.assoc "shape_version" xml.Xml.attributes))
      with
      | Not_found
      | Termcode.InvalidShape ->
        None
    ...
    Debug.dprintf debug "[Info] build_session: ~sum_shape_version is %a@\n"
      (Pp.print_option Termcode.pp_sum_shape_version)
      sum_shape_version;
  ...
  | s -> Loc.warning "Session.load_session: unexpected element '%s'@." s
...
module Goal_Shape = struct
  ...
  let shape (id, s) =
    ...
    with
    | Not_found -> (Termcode.empty_shape, Termcode.Gshape.empty_bshape)
...
let save_detached_proof s parent old_pa_n =
  ...
  ignore
    (add_proof_attempt s old_pa.prover old_pa.limit old_pa.proof_state
       ~obsolete:old_pa.proof_obsolete old_pa.proof_script parent)
...
let rec save_detached_goal old_s s parent detached_goal_id id =
  ...
  Hprover.iter
    (fun _ pa ->
      ...
      save_detached_proof s id pa)
    detached_goal.proofn_attempts;
  List.iter
    (save_detached_trans old_s s id)
    detached_goal.proofn_transformations;
...
let merge_proof new_s ~goal_obsolete new_goal _ old_pa_n =
  ...
  ignore
    (add_proof_attempt new_s old_pa.prover old_pa.limit old_pa.proof_state
       ~obsolete old_pa.proof_script new_goal)
...
let is_fatal e =
  Generic_arg_trans_utils.(
    ...
    | Arg_trans_pattern _
    | Arg_trans_type _
    | Arg_bad_hypothesis _
    | Cannot_infer_type _
    | Unnecessary_terms _
    | Remove_unknown _
    | Args_wrapper.Parse_error _
    | Args_wrapper.Arg_expected _
    | Args_wrapper.Arg_theory_not_found _
    | Args_wrapper.Arg_expected_none _
    | Args_wrapper.Arg_qid_not_found _
    | Args_wrapper.Arg_pr_not_found _
    | Args_wrapper.Arg_error _
    | Args_wrapper.Arg_parse_type_error _
    | Trans.Unnecessary_arguments _
    | Reflection.NoReification ->
      false
    | _ -> true)
...
let apply_trans_to_goal ~allow_no_effect s env name args id =
  ...
  (* If any generated task is equal to the former task, then we made no progress
     because we need to prove more lemmas than before *)
  match subtasks with
  | [ t' ] when Task.task_equal t' task && not allow_no_effect ->
    Debug.dprintf debug
      "[apply_trans_to_goal] apply_transform made no progress@.";
    raise NoProgress
  | _ -> subtasks
...
let add_registered_transformation s env old_tr goal_id =
  ...
  try
    (* check if transformation already present with the same parameters. this
       should always fail and raise Not_found *)
    let _tr =
      List.find
        (fun transID ->
          (get_transfNode s transID).transf_name = old_tr.transf_name
          && Lists.equal ( = ) (get_transfNode s transID).transf_args
               old_tr.transf_args)
        goal.proofn_transformations
    in
    Printexc.print_backtrace stderr;
    Loc.warning
      "[add_registered_transformation] FATAL transformation already present@.";
    exit 2
  with
  | Not_found ->
    let subgoals =
      apply_trans_to_goal ~allow_no_effect:true s env old_tr.transf_name
        old_tr.transf_args goal_id
    in
...
let rec merge_goal env new_s old_s ~goal_obsolete old_goal new_goal_id =
  ...
  new_goal_node.proofn_transformations <-
    List.rev new_goal_node.proofn_transformations;
...
and merge_trans env old_s new_s new_goal_id old_tr_id =
  ...
  let old_subtasks = List.map (fun id -> (id, old_s)) old_tr.transf_subtasks in
  try
    match
      (* add_registered_transformation actually apply the transformation. It can
         fail *)
      try Some (add_registered_transformation new_s env old_tr new_goal_id) with
      | e when not (Debug.test_flag Debug.stack_trace) ->
        (* Non fatal exception are silently ignored *)
        if is_fatal e then
          Loc.warning
            "FATAL unexpected exception during application of %s: %a@."
    ...
    | Some new_tr_id ->
      ...
      let new_subtasks =
        List.map (fun id -> (id, new_s)) new_tr.transf_subtasks
      in
        ...
        (function
          | (new_goal_id, _), Some ((old_goal_id, _), goal_obsolete) ->
            merge_goal env new_s old_s ~goal_obsolete
              (get_proofNode old_s old_goal_id)
              new_goal_id
          | (id, s), None ->
  ...
  | e when not (Debug.test_flag Debug.stack_trace) ->
    ...
    Loc.warning "[Session_itp.merge_trans] FATAL unexpected exception: %a@."
...
let merge_theory env old_s old_th s th : unit =
  let get_goal_name goal_node =
    ...
    try
      ...
      String.concat "." l
    with
    | Not_found -> name.Ident.id_string
  in
  ...
  List.iter
    (fun ng_id ->
      try
        ...
        in
        if goal_obsolete then found_obsolete := true;
        merge_goal env s old_s ~goal_obsolete old_goal ng_id
      with
      | Not_found ->
        (* if no goal of matching name is found store it to look for matching
           shape *)
  ...
  (* attach the session to the subtasks to be able to instantiate Pairing *)
  let detached_goals =
    Hstr.fold (fun _key g tl -> (g, old_s) :: tl) old_goals_table []
  in
  let associated, detached =
    if Termcode.is_bound_sum_shape_version old_s.shapes.shape_version then
  ...
  th.theory_goals <-
    th.theory_goals @ save_detached_goals old_s detached s (Theory th)

(* add a theory and its goals to a session. if a previous theory is provided in
   merge try to merge the new theory with the previous one *)
let make_theory_section ?merge ~detached (s : session) parent_name
    (th : Theory.theory) : theory =
  let tasks = Task.split_theory th None None in
  let goalsID = List.map (fun _ -> gen_proofNodeID s) tasks in
  let theory =
    {
      theory_name = th.Theory.th_name;
  ...
  (match merge with
  | Some (old_ses, old_th, env) -> merge_theory env old_ses old_th s theory
  | _ -> if tasks <> [] then found_detached := true)
...
let add_file_section (s : session) (fn : string) ~file_is_detached
  ...
  Debug.dprintf debug "[Session_itp.add_file_section] fn = %a@."
    Sysutil.print_file_path fn;
  (* if Hfile.mem s.session_files fn then begin Printexc.print_backtrace stderr;
     Loc.warning "[session] FATAL: file %s already in database@." fn; exit 2 end
     else *)
  ...
  let theories =
    List.map (make_theory_section ~detached:false s fid) theories
  in
  f.file_theories <- theories;
  f

(* add a why file to a session and try to merge its theories with the provided
   ones with matching names *)
let merge_file_section ~old_ses ~old_theories ~file_is_detached ~env
    (s : session) (fn : string) (theories : Theory.theory list) format : unit =
  ...
  let theories, detached =
    ...
    let add_theory (th : Theory.theory) =
      ...
      try
        ...
        Debug.dprintf debug_merge
          "[Session_itp.merge_file_section] theory found: %s@." theory_name;
        Hstr.remove old_th_table theory_name;
        make_theory_section ~detached:false ~merge:(old_ses, old_th, env) s fid
          th
      with
      | Not_found ->
        (* if no theory was found we make a new theory section *)
        Debug.dprintf debug_merge
          "[Session_itp.merge_file_section] theory NOT FOUND in old session: \
           %s@."
          theory_name;
    ...
    let detached =
      Hstr.fold
        (fun _key th tl -> save_detached_theory fid old_ses th s :: tl)
        old_th_table []
    in
...
let read_file env ?format fn =
  ...
  let ltheories =
    Mstr.fold
      (fun name th acc ->
        ...
        let th_name =
          Ident.id_register (Ident.id_derive name th.Theory.th_name)
        in
  ...
  in
  let th =
    List.sort (fun (l1, _, _) (l2, _, _) -> Loc.compare l1 l2) ltheories
  in
...
let merge_file env (ses : session) (old_ses : session) file =
  ...
  let file_name =
    Sysutil.system_dependent_absolute_path (get_dir old_ses) (file_path file)
  in
  Debug.dprintf debug "merging file %s@." file_name;
  try
    let new_theories, format = read_file env file_name ~format in
    merge_file_section ses ~old_ses ~old_theories ~file_is_detached:false ~env
      file_name new_theories format;
    None
  with
  | Loc.Located _ as e ->
    (* TODO: capture only syntax and typing errors *)
    merge_file_section ses ~old_ses ~old_theories ~file_is_detached:true ~env
      file_name [] format;
    Some e
  ...
  session_iter_proof_attempt
    (fun _ pa ->
      ...
      let timelimits, steplimits, memlimits =
        try PHprover.find prover_table prover with
        | Not_found ->
      ...
      let tf =
        try Hashtbl.find timelimits lim_time with
        | Not_found -> 0
      in
      let sf =
        try Hashtbl.find steplimits lim_steps with
        | Not_found -> 0
      in
      let mf =
        try Hashtbl.find memlimits lim_mem with
        | Not_found -> 0
      in
...
let get_prover_to_save prover_ids p (timelimits, steplimits, memlimits) provers
  ...
  in
  let id =
    try PHprover.find prover_ids p with
    | Not_found -> (
      ...
      try
        while true do
          try
            let _ = Hashtbl.find occurs !id in
            incr id
          with
  ...
  in
  Mprover.add p
    (id, mostfrequent_timelimit, mostfrequent_steplimit, mostfrequent_memlimit)
    provers
...
let save_prover fmt id
    (p, mostfrequent_timelimit, mostfrequent_steplimit, mostfrequent_memlimit) =
  let steplimit =
    if mostfrequent_steplimit < 0 then
      None
    else
      Some mostfrequent_steplimit
  in
  fprintf fmt
    "@\n\
     @[<h><prover@ id=\"%i\"@ name=\"%a\"@ version=\"%a\"%a@ \
     timelimit=\"%g\"%a@ memlimit=\"%d\"/>@]"
    id save_string p.Whyconf.prover_name save_string p.Whyconf.prover_version
    (fun fmt s ->
      if s <> "" then fprintf fmt "@ alternative=\"%a\"" save_string s)
    p.Whyconf.prover_altern mostfrequent_timelimit
    (opt pp_print_int "steplimit")
    steplimit mostfrequent_memlimit

let save_string_attrib name fmt s =
  if s <> "" then fprintf fmt "@ %s=\"%a\"" name save_string s

(* let save_option_def name fmt opt = match opt with | None -> () | Some s ->
   save_string_attrib name fmt s *)
...
let save_result fmt r =
  let steps =
    if r.Call_provers.pr_steps >= 0 then
  ...
  fprintf fmt "<result@ status=\"%s\"@ time=\"%.2f\"%a/>"
    ...
    r.Call_provers.pr_time (opt pp_print_int "steps") steps
...
let save_proof_attempt fmt ((id, tl, sl, ml), a) =
  fprintf fmt "@\n@[<h><proof@ prover=\"%i\"%a%a%a%a>" id
    (save_float_def "timelimit" tl)
    a.limit.Call_provers.limit_time
    (save_int_def "steplimit" sl)
    a.limit.Call_provers.limit_steps
    (save_int_def "memlimit" ml)
    a.limit.Call_provers.limit_mem
    (save_bool_def "obsolete" false)
    a.proof_obsolete;
  (match a.proof_script with
  | None -> ()
...
let save_ident fmt id =
  let n =
    try
      ...
      String.concat "." l
    with
    | Not_found -> id.Ident.id_string
  in
  fprintf fmt "name=\"%a\"" save_string n

let rec save_goal s ctxt fmt pnid =
  let pn = get_proofNode s pnid in
  fprintf fmt "@\n@[<v 0>@[<h><goal@ %a%a%a>@]" save_ident pn.proofn_name
    (save_string_attrib "expl")
    pn.proofn_expl
    (save_bool_def "proved" false)
    (pn_proved s pnid);
  let sum = Sshape.find_sum s pnid in
  let shape = Sshape.get_shape s pnid in
  Compress.Compress_z.output_string ctxt.ch_shapes
    (Termcode.string_of_checksum sum);
  Compress.Compress_z.output_char ctxt.ch_shapes ' ';
  Compress.Compress_z.output_string ctxt.ch_shapes
    (Termcode.string_of_shape shape);
  Compress.Compress_z.output_char ctxt.ch_shapes '\n';
  let l =
    Hprover.fold
      (fun _ a acc ->
  ...
  in
  let l =
    List.sort (fun ((i1, _, _, _), _) ((i2, _, _, _), _) -> compare i1 i2) l
  in
  List.iter (save_proof_attempt fmt) l;
  let l =
    List.fold_left
      (fun acc t -> (t, get_transfNode s t) :: acc)
      [] pn.proofn_transformations
  in
  let l =
    List.sort (fun (_, t1) (_, t2) -> compare t1.transf_name t2.transf_name) l
  in
...
and save_trans s ctxt fmt (tid, t) =
  ...
  in
  fprintf fmt "@\n@[<hov 1>@[<h><transf@ name=\"%a\"%a %a>@]" save_string
    t.transf_name
    (save_bool_def "proved" false)
    (tn_proved s tid)
    (Pp.print_list Pp.space save_arg)
    t.transf_args;
...
let save_theory s ctxt fmt t =
  ...
  if t.theory_goals <> [] then (
    fprintf fmt "@\n@[<v 1>@[<h><theory@ %a%a>@]" save_ident t.theory_name
      (save_bool_def "proved" false)
      (th_proved s t);
...
let save_file s ctxt fmt _ f =
  fprintf fmt "@\n@[<v 0>@[<h><file%a%a>@]@\n%a"
    (save_string_attrib "format")
    f.file_format
    (save_bool_def "proved" false)
    (file_proved s f) save_file_path f.file_path;
...
let save fname shfname session =
  ...
  let provers =
    PHprover.fold
      (get_prover_to_save prover_ids)
      (get_used_provers_with_stats session)
      Mprover.empty
  in
  let provers_to_save =
    Mprover.fold
  ...
  if Termcode.is_bound_sum_shape_version session.shapes.shape_version then (
    ...
    let s =
      Format.asprintf "(* shapes version: %a *)\n" Termcode.pp_sum_shape_version
        session.shapes.shape_version
    in
    Compress.Compress_z.output_string chsh s;
    Termcode.Gshape.write_shape_to_file session.shapes.session_global_shapes
      chsh;
    Compress.Compress_z.output_string chsh "\n"
  );
  let ctxt = { provers; ch_shapes = chsh } in
...
let save_session (s : session) =
  let uniformize_shape () =
    ...
    if
      not
        (Hfile.fold
           (fun _ f b -> is_detached s (AFile f) && b)
           (get_files s) true)
    then (
          ...
          | APn g when not (is_detached s any) ->
            ...
            let _, expl, _ =
              let root =
                ...
                | Theory _ -> true
              in
              Termcode.goal_expl_task ~root t
            in
...
let export_as_zip s =
  ...
  if n = 0 then
    ...
    raise (Sys_error ("cannot produce archive " ^ archive))
