...
(* TODO:
   - Check ref (two syntax)
   - Check the handling of 'self' as a local variable in methods.
   - Handle 'match E with | _ -> expr' is compiled as 'ignore E; expr'
   - Constructors should terminate with an explicite record contructor
   - Check multiple imports (generated by clones) 
 *)
 ...
 *)
  ...
  type definition =
    ...
    | DeclMethod of visibility * is_static * ident * type_definition * (ident * type_definition) list * xsymbol list * body
...
(*    | ExprBinOp (BinOpGT,_, _) -> true *)
  ...
  (*    | _ -> false*)
    ...
    let zero_expr = ExprConst (CstInt "0")
    let one_expr = ExprConst (CstInt "1")
    ...
    let simplify_last_statement  = function
      | StmtReturn ExprNop -> StmtNop
      | _ as s -> s

    
    let simplify_statement stmt =
      match elim_empty_blocks (elim_nop stmt) with
      | StmtIf (c, t, e) ->
         mk_if c (simplify_last_statement t) (simplify_last_statement e)
      | StmtSequence (s1, s2) ->
         mk_seq s1 (simplify_last_statement s2)
    ...
    let rec mk_body (d : definition list) (s : statement) : body = 
      match d, s with
      | [], StmtBlock (d, s) -> mk_body d s
      | d, StmtBlock (d', s) -> mk_body (d @ d') s
      | d, StmtSequence (StmtBlock (d', s), s2) ->
         mk_body (d @ d') (mk_seq s s2)
      | _ -> (d, simplify_statement s)
...
module JavaCompilerInfo : sig
  ...
  val init_compiler_info : Pmodule.pmodule -> unit

  exception UnknownClass of string

  val is_interface : unit -> bool
  
  val get_class : Ident.ident -> class_record
  val get_current_class : unit -> class_record
  val is_current_class : class_record -> bool
  val import_module : Pmodule.pmodule -> unit
  val is_imported_module : Pmodule.pmodule -> bool
  
  val set_this_type : Ident.ident -> JavaAST.instance_variable list -> unit
  val get_this_type : unit -> Ident.ident 
  val is_static_class : unit -> bool 
  val get_current_class_name : unit -> Ident.ident
  val is_this_variable : Ident.ident -> Ident.ident -> bool
  val is_this_type : Ident.ident -> bool
  val is_constructor : info -> Ident.ident -> bool
  val is_dummy : Ident.ident -> bool
  val add_method : int option -> Ident.ident -> unit
  val get_method : Ident.ident -> class_method
  val get_method_opt : Ident.ident -> class_method option
  val get_default_visibility : unit -> visibility
  
  exception NoSuchEnumType of string
  exception EnumTypeAlreadyDefined of string

  val is_enum_type : Ident.ident -> bool
  val get_enum_values : Ident.ident -> Sid.t
  val set_enum_values : Ident.ident -> Sid.t -> unit
  val get_enum_owner : Ident.ident -> class_record

  exception ExceptionAlreadyDefined of string
  exception UnknownException of string
  
  val set_exception : Ity.xsymbol -> unit
  val get_exception_class : Ity.xsymbol -> class_record 
  val is_known_java_exception : info -> Ity.xsymbol -> bool
end = struct
  open JavaAST
  open Pmodule
  open Ident
  ...
  (* let is_exception () =
     Opt.inhabited (get_current_class ()).exception_parent *)
  ...
  let set_this_type (id : Ident.ident) (ivs : instance_variable list) =
    ...
    if Opt.inhabited cls.this_type_id then
      Loc.errorm ?loc:id.id_loc "only one type can be declared in classes.";
    assert (not (Opt.inhabited (Mid.find_opt id compinfo.known_classes)));
  ...
  let get_this_type () : Ident.ident =
    Opt.get (get_this_type_opt ())

  let is_static_class () : bool =
    not (Opt.inhabited (get_this_type_opt ()))
  ...
  let is_enum_type (id : Ident.ident) : bool =
    Opt.inhabited (get_enum_values_opt id)
 ...
 *)
  ...
  let print_class_name (info : info) fmt (cls : class_record) =
    if is_imported_module cls.m || not (Opt.inhabited cls.package_name) ||
         is_current_class cls then
      default_ident_printer info fmt cls.class_name
    else
      fprintf fmt "%s.%a" (Opt.get cls.package_name)
  ...
  let rec print_expression ~(prec:int) (info : info) fmt (e : expression) =
    ...
    and print_method_call (e : expression) (mid : Ident.ident)
      ...
      let print_method_ctx fmt e =
        ...
        match e with
        | ExprNop -> assert (not (Opt.inhabited m.this_index));
  ...
  let rec print_statement ?(braces_on=true) (info : info) fmt
    ...
    | StmtThrow (xs, ExprNop) ->
       if is_known_java_exception info xs then
         ...
         fprintf fmt "@[throw new %s ();@]" (Opt.get sopt)
       else
    ...
    | None ->
       if javaex then
         ...
         fprintf fmt "@[catch (%s %a) {@\n%a@]@\n}"
           (Opt.get sopt) (default_ident_printer info) anonymous_exception_id 
  ...
  and print_method (info : info) fmt (vis : visibility) is_static (id : ident)
        (restype : type_definition)
        (args : (ident * type_definition) list)
        (exlist : xsymbol list)
        (body : body) =
    fprintf fmt "@[@[<2>%a%a%a %a(@[%a@])%a {@\n@[%a@]@]@\n}@]"
      print_visibility vis
  ...
  and print_definition (info : info) fmt (def : JavaAST.definition) =
    ...
    | DeclMethod (vis, is_static, id, restype, parameters, exlist, body) ->
       print_method info fmt vis is_static id restype parameters exlist body
...
module MLW2Java = struct
  ...
  let translate_record_def (info : info) (id : ident)
    ...
    if is_static_class () then
      begin
        ...
        List.map (fun iv -> DeclInstanceVar iv) fields
      end
    else
  ...
  let translate_type_decl (info : info) (tdef : Mltree.its_defn)
    ...
    if Sattr.mem java_ignore type_id.id_attrs then
      []
    else if not (Opt.inhabited tdef.its_def) then
      begin
        set_this_type type_id [];
        []
      end
    else match Opt.get tdef.its_def with
         ...
         | Dalias t ->
            (* TODO: how to handle "type t = { a : some_type }" ?
               It is translated as type t = some_type *)            
    ...
    let rec aux vl res has_unit this_index =
      ...
      | (id, ty, is_ghost) :: tl ->
         ...
         | Tapp (typeid, []) when is_this_variable id typeid ->
            ...
            else if not (Opt.inhabited this_index) then
...
(env : translation_env) : body =
  ...
  let translate_let_decl (info : info) (ldef : let_def)
    ...
    let translate_method_proto ~(is_cstr:bool) rs stv restype varlist =
       ...
       let jrestype = match restype with
         | Tvar _ ->
            Loc.warning "return type of '%s' is set to void."
              rs.rs_name.id_string;
            VoidType
    ...
    | Lsym (rs, stv, restype, varlist, body) when
       ...
       begin
         ...
         | ThisType ->
            ...
            assert (not (Opt.inhabited this_index));
    ...
    | Lsym (rs, stv, restype, varlist, body) ->
       ...
       begin
         ...
         let static = not (Opt.inhabited this_index) in 
         ...
         [ DeclMethod (visibility, static, rsid, jrestype, vlist, exlist, jbody) ]
       end
    ...
    | Lany (rs, stv, restype, varlist) ->
       ...
       let vlist, this_index, jrestype, visibility, exlist =
         translate_method_proto ~is_cstr:false rs stv restype varlist in
       if not (Opt.inhabited this_index) then
         Loc.errorm ?loc:rsid.id_loc
    ...
    | Lrec ([{rec_sym = rs; rec_args = varlist; rec_exp = body;
              rec_res = restype; rec_svar = stv }]) ->
       let rsid = rs.rs_name in 
       if is_interface () then
         Loc.errorm ?loc:rsid.id_loc
           "concrete methods are not allowed in interface";
    ...
    | Lrec (_) -> unsupported "empty let rec"
  ...
  let translate_exception_decl (info : info) (exc : xsymbol) (t : ty option) =
    try
       match translate_type info (Opt.get t) with
  ...
  let print_prelude _ ?old:_ ?fname:_ ~deps ~global_prelude ~prelude fmt m =
    ...
    let print_deps fmt deps =
      ...
      let deps = List.filter
                   (fun m -> Opt.inhabited (get_module_package_name m)) deps
      in 
