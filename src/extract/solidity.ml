...
module Print = struct
  ...
  let print_ity ~use_quote ?paren info fmt ity =
    print_ty ~use_quote ?paren info fmt (Compile.Translate.mlty_of_ity_hack ity)
  ...
  let check_val_in_drv msg info loc id =
    (* here [rs] refers to a [val] declaration *)
    match query_syntax info.info_syn id with
    | None (* when info.info_flat *) ->
        Loc.errorm ?loc "Symbol %a cannot be extracted for solidity: %s" (print_lident info) id msg
    | _ -> ()

  let is_mapped_to_uint info ity =
    match ity.ity_node with
    | Ityapp ({ its_ts = ts }, _, _) ->
        query_syntax info.info_syn ts.ts_name = Some "uint"
    | _ -> false
  ...
  let rec print_apply_args info fmt (exprl,_) =
    Format.fprintf fmt "(%a)" (Pp.print_list Pp.comma (print_expr info 3)) exprl
  ...
  and print_fun_type_args info fmt (args, _s, res, e) =
    let print_res fmt = function
      | Ttuple [] -> ()
      | _ ->
    ...
    let print_args fmt = function
      | [(_,Ttuple [],_) ] -> ()
      | args ->
          Pp.print_list comma (print_vs_arg info) fmt args
    in
    fprintf fmt "@[(%a)@]%a@ {@ @[<v>%a@] }"
      print_args args
      print_res res
  ...
  and print_let_def ?(functor_arg=false) info fmt = function
    | Lvar (pv, e) ->
        fprintf fmt "@[<hov 2>%a %a =@ %a;@]"
          (print_ity ~use_quote:false info) pv.pv_ity
          (print_lident info) (pv_name pv) (print_expr ~opr:false info 18) e
    | Lsym (rs, svar, res, args, ef) ->
        if not (Stv.is_empty svar) then
          Loc.errorm ?loc:rs.rs_name.id_loc "Unsupported: polymorphic functions";
    ...
    | Lany (rs, _svar, res, args) when functor_arg ->
        ...
        fprintf fmt "@[<hov 2>val %a : @[%a@] ->@ %a@]"
          ...
          (print_ty ~use_quote:true info) res;
        forget_vars args
    | Lany ({rs_name}, _, _, _) -> check_val_in_drv "Lany" info rs_name.id_loc rs_name
  ...
  and print_expr ?(boxed=false) ?(opr=true) ?(be=false) info prec fmt e =
    ...
    | Econst (Constant.ConstInt c) ->
        ...
        (match query_syntax info.info_literal id with
         ...
         | None   -> fprintf fmt (protect_on (prec < 4) "%s") n)
    | Econst (Constant.ConstStr s) ->
        Constant.print_string_def fmt s
    ...
    | Ematch (_, _, []) ->
        Loc.errorm "Match not suported in solidity"
    | Eassign al ->
    ...
    | Eif (e1, e2, e3) ->
        fprintf fmt (protect_on (opr && prec < 16)
          ...
          (print_expr ~be:false info 15) e3
    | Eblock [] ->
        fprintf fmt "()"
    | Eblock [e] ->
    ...
    | Eraise ({xs_name = { id_string="'Return"}}, opt_arg) ->
        fprintf fmt "@[<hv 2> return %a;@]"
          (Pp.print_option (print_expr info 15)) opt_arg
    | Eraise ({xs_name = { id_string; id_loc }}, _) ->
        Loc.errorm ?loc:id_loc "Raise can't be extracted: %s"
          id_string
    | Efor (pv1, pv2, To, pv3, e) ->
        if is_mapped_to_uint info pv1.pv_ity then begin
          fprintf fmt "@[<hv 2>for(uint %a = %a; %a <= %a; %a++){@ @[%a@]@ }@]"
          ...
          Loc.errorm "for loop is implemented only on uint"
    | Efor (_, _, DownTo, _, _) ->
        Loc.errorm "Downto for loop unimplemented"
    | Ematch (e, [], _) ->
        (* no raise, so no catch *)
        print_expr ~be:true ~opr:false info prec fmt e
    | Ematch (e, bl, _) ->
        fprintf fmt
          (if (prec < 18 && opr)
    ...
    | Eexn (_, _, e) ->
        (* no raise so no need to define them *)
  ...
  let print_type_decl info _fst fmt its =
    ...
    let print_field fmt (_is_mutable, id, ty) =
      fprintf fmt "%a %a;"
        (print_ty ~use_quote:true ~paren:false info) ty
        (print_lident info) id in
  ...
  let print_top_val ?(functor_arg=false) info fmt ({pv_vs}, ty) =
    if functor_arg then
      ...
      check_val_in_drv "print_top_val" info pv_vs.vs_name.id_loc pv_vs.vs_name
  ...
  let rec print_decl ?(functor_arg=false) info fmt = function
    ...
    | Dval (pv, ty) ->
        fprintf fmt "@[<hov 2>%a %a;@]"
          (print_ty info ~use_quote:false) ty
          (print_lident info) pv.pv_vs.vs_name
    | Dtype dl ->
        print_list_next newline (print_type_decl info) fmt dl
    | Dexn (_, _) ->
        (* no raise so no need for exception *)
        ()
    | Dmodule (s, dl) ->
...
let ocaml_printer = Pdriver.{
    desc = "printer for Solidity code";
    implem_printer = {
        filename_generator = ng ".sol";
...
let () = Pdriver.register_printer "solidity" ocaml_printer
