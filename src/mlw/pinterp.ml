...
let check_term ?vsenv env (ctx: pre_cntr_ctx) t =
  ...
  let task_filename = match Sys.getenv "WHY3RACTASKDIR" with
    | temp_dir ->
...
let set_limits () =
  if !limits_state <> None then failwith "with_limits: called twice";
  limits_state := Some (Sys.time (), ref 0)

let reset_limits () =
  Debug.dprintf debug_trace_exec
    "Finished after %d steps@." !(snd (Opt.get !limits_state));
  limits_state := None

let with_limits f =
  set_limits ();
  match f () with
  | res -> reset_limits (); res
  | exception e ->
...
module Sidpos = struct
  ...
  (** Currently the model can contain only one value for a variable defined in a
     ...
     TODO In the long-term, [Model_parser.model] should retain the identifiers
     of the variables, because these are of course different for variables that
     have been duplicated by cloning. *)
  let check locs id =
    match id.id_loc with
    | Some loc when mem (id, loc) locs ->
        cannot_compute "ambigous global at %a" Pretty.print_loc' loc
    | _ -> ()
end
