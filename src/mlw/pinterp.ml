        ...
        | Some ty ->
          begin try
            ...
            | _ ->
              ...
              {ctx with env= bind_ls ls v ctx.env}, Sidpos.add_id id locs
          with
          (* Do not fail if [get_and_register_global] returns Incomplete exception
             because the value is missing *)
          | Incomplete s -> Debug.dprintf debug_rac_values "[bind_globals] catching Incomplete %s with ls = %a@." s print_ls ls; ctx, locs
          end
        end
      | _ -> ctx, locs
      end
